...
class PitchCounterView extends WatchUi.View {
    ...
    public function onUpdate(dc as Dc) as Void {
        if (_labelCount != null) {
            _labelCount.setText("Count: " + _pitchCounter.getCount());
        }

        if (_labelSamples != null) {
            _labelSamples.setText("Samples: " + _pitchCounter.getSamples());
        }

        if (_labelPeriod != null) {
            _labelPeriod.setText("Period: " + _pitchCounter.getPeriod());
        }
        View.onUpdate(dc);
    }
