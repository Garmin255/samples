...
class PitchCounterProcess {
    ...
    private var _x as Array<Float> = [0.0];
    private var _y as Array<Number> = [0];
    private var _z as Array<Number> = [0];
    ...
    public function initialize() {
        ...
        try {
            ...
            _session = ActivityRecording.createSession({:name=>"PitchCounter", :sport=>Activity.SPORT_GENERIC, :sensorLogger =>_logger as SensorLogger});
        } catch (e) {
    ...
    public function accelCallback(sensorData as SensorData) as Void {
        ...
        if (accelData != null) {
            if (_filter != null) {
                _x = _filter.apply(accelData.x as Array<Numeric>);
            }
            _y = accelData.y;
            _z = accelData.z;
            onAccelData();
