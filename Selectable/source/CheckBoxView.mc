...
class CheckBoxList {
    ...
    private var _list as [CheckBox, CheckBox, CheckBox];
    ...
    public function initialize(dc as Dc) {
        ...
        var width = dims[0].toNumber();
        var height = dims[1].toNumber();
        ...
        if (dc.getHeight() > dc.getWidth()) {
            ...
            initX = (dc.getWidth() / 2) - (width / 2);
        } else {
            slideSymbol = :locX;
            spacing = (dc.getWidth() / 4);
            offset = (width / 2);
            initX = spacing - offset - BORDER_PAD;
            initY = (dc.getHeight() / 2) - (height / 2);
        }
