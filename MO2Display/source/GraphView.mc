...
class GraphView extends WatchUi.View {
    ...
    public function initialize(sensor as MO2Sensor, index as Number) {
        ...
        _fonts = [Graphics.FONT_SMALL, Graphics.FONT_MEDIUM, Graphics.FONT_LARGE];
    }
    ...
    public function onUpdate(dc as Dc) as Void {
        ...
        if (_sensor.getData().getTotalHemoConcentration() != null) {
            ...
            _totalGraph.draw(dc, [(width / 4) + 2, 3], [width - 3, (height / 2) - 3]);
            _totalGraph.draw(dc, [(width / 4) + 2 + 1, 3], [width - 3 + 1, (height / 2) - 3]);
            _totalGraph.draw(dc, [(width / 4) + 2, 3 + 1], [width - 3, (height / 2) - 3 + 1]);
            _totalGraph.draw(dc, [(width / 4) + 2 + 1, 3 + 1], [width - 3 + 1, (height / 2) - 3 + 1]);
        }
        ...
        if (_sensor.getData().getCurrentHemoPercent() != null) {
            ...
            // We should have thick lines eventually... this is a hack for now.
            _currentGraph.draw(dc, [(width / 4) + 2, height / 2], [width - 3, height - 3]);
            _currentGraph.draw(dc, [(width / 4) + 2 + 1, height / 2], [width - 3 + 1, height - 3]);
            _currentGraph.draw(dc, [(width / 4) + 2, height / 2 + 1], [width - 3, height - 3 + 1]);
            _currentGraph.draw(dc, [(width / 4) + 2 + 1, height / 2 + 1], [width - 3 + 1, height - 3 + 1]);
        }
