...
class BackgroundTimerView extends WatchUi.View {
    ...
    public function initialize(backgroundRan as PersistableType) {
        ...
        if (backgroundRan == null) {
            ...
            _timerStartTime = Storage.getValue(TIMER_KEY_START_TIME) as Number?;
            _timerPauseTime = Storage.getValue(TIMER_KEY_PAUSE_TIME) as Number?;
        } else {
    ...
    public function onUpdate(dc as Dc) as Void {
        ...
        if (_timerStartTime != null) {
            if (_timerPauseTime != null) {
                ...
                elapsed = -_timerStartTime + Time.now().value();
            }
    ...
    public function startStopTimer() as Void {
        ...
        if (_timerStartTime == null) {
            _timerStartTime = now;
            _updateTimer.start(method(:requestUpdate), 1000, true);
        } else if (_timerPauseTime == null) {
            _timerPauseTime = now;
            _updateTimer.stop();
            WatchUi.requestUpdate();
        } else if ((_timerPauseTime - _timerStartTime) < _timerDuration) {
