...
class ApplicationStorageViewDelegate extends WatchUi.BehaviorDelegate {
    ...
    //! Set or delete the next storage item
    //! @return true if handled, false otherwise
    public function processNextStorageItem() as Boolean {
        switch (_count) {
            case $.KEY_INT:
            ...
            case $.KEY_BOOLEAN:
                ...
                break;
            case $.KEY_ARRAY:
                Storage.setValue(_count, [1, 2, 3, null]);
                break;
            case $.KEY_DICTIONARY:
                Storage.setValue(_count, {1=>"one", "two"=>2, "null"=>null});
                break;
            default:
